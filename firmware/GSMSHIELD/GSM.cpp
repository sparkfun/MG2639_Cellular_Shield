...
//This is how the software pins are mapped on the MG2639 shield
...
/*
    Sends the AT command to module to see if it responds. If not, turn on module and try again
...
int GSM::begin(long baud_rate)
{
	...
	_cell.begin(baud_rate); //Start communication with module at user selected baud rate

	p_comm_buf = &comm_buf[0];

	setStatus(IDLE);

	//Try communicating with module. If not response, turn on module
...
#ifdef DEBUG_ON
			Serial.println(F("GSM Init: No Response, turning on module"));
#endif
...
#ifdef DEBUG_ON
		    Serial.println(F("GSM Init: Module responded"));
#endif
            WaitResp(1000, 1000); //Why is this here?
            break; //If module responds, break - we are good to go
        }
    }
...
#ifdef DEBUG_ON
        Serial.println(F("DB: Module responded"));
#endif
        turnedON = true;
        noresponse = false;
    }
...
#ifdef DEBUG_ON
          Serial.println(F("DB: Brute forcing baud rate"));
#endif
          for (int i = 0 ; i < 8 ; i++) {
               switch (i) {
               case 0:
               ...
               if (AT_RESP_OK == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)) {
                    ...
                    if (AT_RESP_OK == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)) {
#ifdef DEBUG_ON
                         Serial.print(F("DB:Module communicating at "));
...
#ifdef DEBUG_ON
               Serial.println(F("DB:Module failed to respond to any baud rate"));
#endif
          }
    ...
    if(noresponse == true && !turnedON) {
        ...
        Serial.println(F("ERROR: The cell module didn't answer. Check power and serial pins in GSM.cpp"));
...
/*
 Set a bunch of default settings for this particular module
 
 Most of these commands should work on different module manufacturers
...
void GSM::InitParam(byte group)
{
    ...
    case PARAM_SET_1:
        ...
        // select phonebook memory storage - Save to SIM card
        SendATCmdWaitResp(F("AT+CPBS=\"SM\""), 1000, 50, str_ok, 5);

        //Close GPRS PDR context?
...
/*
    ...
    Full function - partial function below
    Given an max timeout in ms, what that amount of time
    Or what for a max number of characters to be received
  
    Then check if the string that was received was the string we were expecting
    
return: RX_FINISHED_STR_RECV - the expected string was returned
        RX_FINISHED_STR_NOT_RECV - a string was received but not the one expected
        RX_TMOUT_ERR - a max time was exceeded
*/
                   ...
                   char const *expected_resp_string)
{
    ...
    RxInit(start_comm_tmout, max_interchar_tmout); //Clear buffer and start timers
...
/*
    ...
    Wait for a response of some kind - we don't check what kind

return: RX_TMOUT_ERR - we hit start reception timeout
        RX_FINISHED - we hit interchar_timout
        RX_NOT_FINISHED - we hit neither timeout
*/
byte GSM::WaitResp(uint16_t start_comm_tmout, uint16_t max_interchar_tmout)
{
    byte status;

    RxInit(start_comm_tmout, max_interchar_tmout); //Clear buffer and set timers

    // wait until response is not finished
    do {
        status = IsRxFinished(); //Check to see if timers have expired
    } while (status == RX_NOT_FINISHED);
...
/*
    ...
    Reset the RX buffer and various circular buffer variables
    Set the max time for:
        amount of time before first character is received
        amount of time between characters before bailing
*/
...
/*
    ...
    State machine for seeing if we have timed out from receiving nothing,
    of if we have timed out betwee receiving characters
    
    Based on start_comm_tmout and max_interchar_timout
    Run until we hit max times
...
byte GSM::IsRxFinished(void)
{
    ...
    if (rx_state == RX_ALREADY_STARTED) {
        ...
        if (num_of_bytes) prev_time = millis(); //If there are new bytes, reset timer

        // read all received bytes
        while (num_of_bytes) {
            num_of_bytes--;
            if (comm_buf_len < MAX_COMM_BUF_LEN) {
...
/**********************************************************
Compare a given string against what is in the comm_buff

compare_string - pointer to the string we are looking for
...
*/
        ...
        if (ch != NULL) {
            ...
            Serial.print("Looking for string: \"");
            Serial.print(compare_string);
            Serial.print("\"- Received: \"");
        ...
        //The comm_buf_len is zero so there is nothing received
#ifdef DEBUG_ON
        Serial.print(F("Looking for string: \""));
        Serial.print(compare_string);
        Serial.println(F("\" - No string received"));
#endif
    }
